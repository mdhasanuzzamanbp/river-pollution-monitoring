# Selected Figures
<div align="justify">
For most of the images, please visit the Scopus journal website: https://onlinelibrary.wiley.com/doi/full/10.1002/etc.4814
</div>
<br>
<p align="center">
<img width="744" height="608" alt="Physiochemical" src="https://github.com/user-attachments/assets/3fdf4daf-e48f-4a3d-b894-907dee13c913" />
</p>
<p align="center">
Fig 1: Spatial variations of the mean physiochemical profiles in the three segments of up, lower and downstream river system, Bangladesh. The bar associated with the mean values represent standard deviations. Different letters (a, b, and c) at the top mean statistically significant spatial differences of the metals based on a one-way ANOVA test (p<0.05).
</p>
<br>
<div align="justify">
The basic statistics (mean and standard deviation) of physicochemical parameters in sediments are outlined in the table. The pH of the sediment varied within 6.5 to 7. Low pH in water can cause the release of absorbed metals, which are found in the upstream river (Mean: 6.23 ±0.76) because it reduces the strength of metal connection and interrupts the tension of metals in surface sediments. The maximum and minimum average pH values were estimated as 5.84-7.25 in the upstream river, 6.51-7.26 in the lower midstream river and 8.01-8.67 in the downstream river sediments, respectively. According to these findings, sediments were slightly alkaline to slightly acidic. The high percentage (%) of organic matter (OM) is detected in the upstream river (Mean: 2.63 ±0.54). The low percentage (%) of OM was found in the downstream river e.g., Pashur and Rupsha River (Mean: 1.32 ±0.91). The TOC in sediments was mainly averaged 1.87 ±0.54, 1.91 ±0.39 and 1.87 ±0.39 in upstream, lower midstream and downstream rivers, respectively.
</div>
  <br>
<p align="center">
<img src="https://github.com/user-attachments/assets/5d7a2c0a-b8e8-4678-909e-7a636aca5621" alt="RSP+Monte Carlo Final Edit" width="600">
</p>
<p align="center">
Fig 2: (a) Metal chemical fraction in riverine sediments for 30 sampling sites using (b) Donut risk chart of the respective metals in up-mid-downstream river systems
</p>
<div align="justify">
The BCR-originating metal fractions influencing bioavailability and bioaccumulation are presented in Fig. 4.2a. All metals were predominantly concentrated in the exchangeable fraction (F1) except for Cd and Pb, which were found in the residual fraction (F4). Average Res. Cd and Res. Pb percentages were 43 and 36.9 % respectively while other metals exchangeable percentages were 41.77% 30 (Exc.Mn), 39.33% (Exc.Fe), 39.57% (Exc.Ni), 42.83% (Exc.Cr), 43.63 % (Exc.Zn), 41.4% (Exc.Cu), 42.37% (Exc.As) respectively (Fig. 2a). Fig 2b in general, metal exchangeable fraction contributions in the riverine sediment followed: Ni > Cr > Zn> As> Fe>Cu>Cd>Mn>Pb. Similarly, organic fraction contributions were in order: Fe>Cu>Mn>Cd>Zn>Ni>As>Cr>Pb. By contrast, metal residual fraction contributions are in the descending order: Pb>Cd>Cu>As>Zn>Fe>Mn>Ni>Cr. Likewise, reducible fraction contributions in the sediment followed: Mn>Cr>Pb>Fe>Zn>As>Ni>Cd>Cu.
</div>
<br>
<p align="center">
<img width="643" height="529" alt="Heatmap Fraction Final Without Monte Carlo Edit" src="https://github.com/user-attachments/assets/81e70497-32b4-4c4f-bb88-049da437a76b" />
</p>
<p align="center">
Fig 3: Heatmap showing metal chemical fractions in riverine sediments for 30 sampling sites in the up-mid-downstream river systems
</p>
<div align="justify">
The figure shows Manganese and Fe were largely associated with F2 and F3, with an average of 24.78% and 28.39 %, respectively.The rest of the metals were most likely abundant in the residual fraction (F4): Pb = 40.45%, and Cd = 36.39%. The large percentage related to F4 indicates a comparatively lower ecological risk. As overall view several sites (1,2; 18,23;10,30 etc.) have hierarchal relationship.
</div>
<p align="center">
<img width="2970" height="1460" alt="Igeo0" src="https://github.com/user-attachments/assets/7e5a1b9c-6caf-4791-8255-3d3b66a564d8" />
</p>
<p align="center">
Fig 4: Geo-accumulation (Igeo) of respective metals in riverine sediment
  </p>
<div align="justify">
As seen from this figure, the Igeo of Cd was the largest, with 16.7% below the uncontaminated to moderately contaminated level, 10% at the moderately contaminated level, 16.7% at the moderately to heavy contaminated level, 46.6% at the heavily contaminated level and 10% at the heavily to extremely contaminated level. In addition to this, both As, and Pb reached a moderate contamination level of 20%. 
</div>
<p align="center">
<img width="1329" height="1183" alt="ChatGPT Image Jul 27, 2026, 09_19_02 AM" src="https://github.com/user-attachments/assets/ff236734-7a85-4e11-a400-d0a5bde201af" />
</p>
<p align="center">
Fig 5: Contributions of respective metals to the Risk Index (RI)
</p>
<div align="justify">
Approximately 48.5% of Cd is attributed to the potential ecological risk (RI) in the up-mid-downstream river segments. The high Cd Igeo values also triggered a high RI index. Further, Ni and As contributed 26% and 21 % of the potential ecological risk, respectively.
</div>
<p align="center">
<img width="307" height="255" alt="EF" src="https://github.com/user-attachments/assets/a5e0e6cc-2d10-4809-9daa-63ef2d30ab1f" />
</p>
<p align="center">
Fig 6: Contribution of Enrichment Factor (EF) of respective metals in riverine sediment
</p>
<div align="justify">
As shown from the fig that, the EF index of Cd was the highest, with 10% at minor enrichment, 6.7 % at moderate enrichment, 10 % at moderately severe enrichment, 26.7 % severe enrichment, 46.7 % at very severe to extremely severe enrichment. In this study, EF values were between 1 and 3 (1 ≤ EF < 3; minor enrichment) in most of the river segments. Moreover, Pb, As, and Cu reached an extremely severe enrichment of 20%, 16.7 and 16.7 %, respectively.
</div>
<p align="center">
<img width="1448" height="1086" alt="ChatGPT Image Jul 27, 2026, 09_12_09 AM" src="https://github.com/user-attachments/assets/ef0cb838-9e7b-4e70-aac7-22f9c4e6eab9" />
</p>
  <p align="center">
  Fig 7: Contributions of respective metals to the Toxicity Unit (TU)
    </p>
<div align="justify">
The toxicity and total toxicity contribution percentage among nine metals, the total STU index of metals in the sediments were followed in the descending order: Zn>Cd>As>Pb>Cr>Ni and Cu. Moreover, Zn, Cd and As contributed 17.5%, 16 % and 15.8% of total STU risk, respectively. 
</div>
<p align="center">
<img width="690" height="608" alt="FactorContribution 1 (%)" src="https://github.com/user-attachments/assets/beaf2dbb-9b67-48c0-aa86-cd477b656606" />
</p>
<p align="center">
 Fig 8: Contribution of four apportioning factors for each metal determined by the PMF model.
</p>
<div align="justify">
In details of, the PMF model outcomes for sources, and factor profiles are presented in the fig. Factor 1 was principally represented by Cu and had a 78.9% contribution. Factor 1 also showed a high contribution to arsenic (62.8%). Lead, Cr, and Zn exhibited a low contribution by factor 1 of 16.2%, 18.1%, and 21.9%, respectively. Factor 2 was primarily characterized by Ni and Mn with 96.9% and 83.5% contributions, respectively. Factor 2 also exhibited a medium contribution to Pb (39.7%) and Zn (38.3%). Factor 3 was mostly characterized by Cd and had a 67.4% contribution. Iron and Cr showed a high contribution by factor 4 of 53.5 % and 64.3%, respectively.
</div>
<p align="center">
  <img width="549" height="608" alt="Conc+Regression 1" src="https://github.com/user-attachments/assets/fbc2a3b8-e771-4587-8f58-bd6e09b4b1bf" />
</p>
<p align="center">
  Fig 9: Measured and predicted metal concentrations (mg Kg-1) in surface sediments at significant level (p < 0.01) (Left panels). Black and red lines represent measured and predicted concentrations. Linear regression results are related to a one-to-one function line (y=x) in right panels. Red line denotes linear regression outcomes whereas black dashed line signifies one-to-one function.
 </p> 
<div align="justify">
Predicted metal datasets were compared to measured metal datasets based on the PMF model the fig. The r2 values were utilized to appraise the efficiency of the PMF model predictions. All metals depicted rational linear regression outcomes with r2 values higher than 0.592. The greater r2 values were detected for Cd (0.996), Mn (0.995), Fe (0.969) and Cr (0.828), demonstrating that the metal datasets forecasted by the preferred source factors were well described in the PMF model. The r2 values for Zn were only 0.137 when all datasets were considered; however, it was strongly affected by a single outlier at location S13 (631.4 mg kg−1 and 4 times higher than the mean Zn concentration)
</div>
<p align="center">
 <img width="685" height="608" alt="Spplementary Regression 2" src="https://github.com/user-attachments/assets/0e6a5da4-37b7-499f-938e-30b1fa95e23c" />
</p> 
<p align="center">
Fig 10: Comparison of measured and predicted concentrations of Zn in surface sediments by the PMF (Left panels). Red line denotes linear regression results and dashed line denotes one-to-one function (y=x). Upper panels (a, b) show data without an outlier and lower panels (c, d) show data using the entire dataset
  </p> 
<div align="justify">
In this study, the location S13 was positioned in the industrial area surround by urban area and agricultural land use, where the non-point source can be detected. The enormously high Zn concentration may be ascribed to a sudden urban runoff source and thus the high Zn was not related to exhibit land-use patterns. 
</div>
  
# References

